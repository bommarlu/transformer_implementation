...
class TestReLULayer:
    def test_gradient_ReLU(self):
        ...
        gradients = relu.backward(np.ones((100, 200)))

        predicted_delta = x_out + (delta * gradients)
        assert not np.any(np.abs((predicted_delta - dx_out)) > 0.01)
...
class TestFullyConnectedLayer:
    def test_input_gradient_fc(self):
        ...
        gradients = fc_layer.backward(np.ones((num_samples, num_output)))

        predicted_delta = loss(x_out) + np.sum(gradients) * delta
        assert np.abs(predicted_delta - loss(dx_out)) < 0.01
...
class TestLayerNorm:
    def test_gradient_layernorm(self):
        ...
        gradients = norm_layer.backward(np.ones((100, 200)))

        predicted_delta = out_x + (delta * gradients)
        assert not np.any(np.abs((predicted_delta - out_dx)) > 0.01)

class TestSoftmax:
    
    def test_gradient_softmax(self):
        def loss(output):
            return np.sum(np.square(output))
         
        delta = 0.5
        in_x = np.random.rand(100, 200)
        in_dx = np.copy(in_x)
        in_dx[0,0] += delta
        ...
        assert np.abs(predicted_delta - loss(out_dx)) < 0.01
